/**
 * security.js — Защита от SQL-инъекций и XSS-атак
 *
 * ПРИМЕЧАНИЕ: Supabase JS Client использует PostgREST API, что означает,
 * что все запросы к БД передаются через HTTP-параметры, а не через
 * конкатенацию строк SQL. Параметры .eq(), .insert(), .update() и другие
 * методы автоматически экранируются на стороне PostgREST, исключая
 * возможность SQL-инъекции. Данный модуль добавляет дополнительный уровень
 * защиты: валидацию на уровне приложения и защиту от XSS при выводе данных.
 */
